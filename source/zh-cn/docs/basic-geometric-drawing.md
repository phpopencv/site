...
在本篇教程中你将学到：
 ...
 - 使用`cv::circle`绘制一个圆
<del> - 使用`cv::fillPoly`绘制一个填充的多边形 </del>
...
这样我们就定义了一个BGR表示的颜色参数，其中:B(Blue)的值为$a,G(Green)的值为$b,R(Red)的值为$c
...
use function CV\{ellipse, circle, fillPoly, rectangleByPoint, line, imshow, moveWindow, waitKey};
...
2.我们创建函数来绘制不同的几何图形。 例如，我们使用`MyEllipse`和`MyFilledCircle`绘制原子图：
   ...
   MyFilledCircle($atom_image, new Point(w/2, w/2));
  ```
3.为了绘制国际象棋中的车，我们使用`MyLine`, `rectangleByPoint` 和 <del> `MyPolygon` </del>:
   ``` php
    <del> //MyPolygon($rook_image); //todo fillPoly </del>
    rectangleByPoint( 
           $rook_image,
           new Point( 0, 7*w/8 ),
           new Point( w, w),
...
### 运行结果
运行你的程序后会返回如下结果（fillPoly尚待完成）：

![result.png](/images/docs/basic-geometric-drawing/result1.jpg)

![result.png](/images/docs/basic-geometric-drawing/result2.jpg)
